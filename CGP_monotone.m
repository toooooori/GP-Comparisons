...
% CGP_MONOTONE
% Constrained Gaussian Process regression under monotonicity (f' >= 0).
%
% Output:
...
% ============================================================
% 2) Active set selection of derivative constraints
% ============================================================
set_num = 0;
...
th_hd   = 1e-20;
t2      = [];          % active derivative constraint locations
tstar_g = tstar;       % keep full grid
...
while max(prob_y) > th_hd
    ...
    % add to active constraint set
    t2 = [t2; temp_va];

    % GP covariances wrt active constraint set t2
    [K1, K2, K3, KK2] = covGP([log(ell);log(sigma_f)], t1, t2); %#ok<ASGLU>
    % K2  = Cov(f(t1), f'(t2))
    % KK2 = Cov(f'(t2), f(t1))
    % K3  = Cov(f'(t2), f'(t2))
    ...
    [Ks1, Ks2, Ks3, KKs2] = covGP([log(ell);log(sigma_f)], t1, tstar); %#ok<ASGLU>

    % kernel pieces for mixed blocks
    tempK2 = gp_dist(t2'/ell, tstar'/ell);
    % Cov(f'(t2), f'(tstar)) under SE kernel d/dx
    K_3    = sigma_f^2 * exp(-tempK2.^2/2) .* (1/ell^2) .* (1 - tempK2.^2);

    % joint GP over [f(t1); f'(tstar)] and [f'(t2)]
    Sigma_11 = [cov_X, Ks2;
                KKs2,  Ks3];
    Sigma_12 = [K2;
                K_3'];
    ...
    v_Lb  = L_Lb \ Lambda_12;
    Lambda_sig = Lambda_22 - v_Lb' * v_Lb;   % conditional cov

    % posterior of derivative process given data at t1
    mean_dX = (L_K3' \ (L_K3 \ KK2))';      % A
    cov_dX  = cov_X - mean_dX * KK2;        % B
    L_dX    = jitterchol(cov_dX);

    % precompute reusable terms for Gibbs
    L_sig = jitterchol(Lambda_sig);         % chol of Lambda_sig
    R     = jitterchol(K3);                 % chol of K3
    Q     = R' \ (R \ eye(set_num));        % precision of f'(t2)
    Q     = (Q + Q')/2;

    % Gibbs to sample derivative under monotonicity constraint (f' >= 0)
    M    = 501;
    Zd   = zeros(set_num, M);   % raw derivative samples
    ...
    for k = 2:M
        ...
        for i = 1:set_num
            % conditional Normal of f'(t2_i) given others via precision Q
            Qi_i    = Q(i,i);
            ...
            % conditional posterior proposal for f'(t2_i)
            temp_a = mean_dX(:, i);
            if set_num ~= 1
                if i == 1
            ...
            % truncated mixture sampling enforcing f'(t2_i) >= 0
            eps      = 0;
            temp_ks  = normcdf(eps, mu_m(i), nu_sqrt);              % mass below 0 (unconstrained)
            temp_qx  = 1 - normcdf(eps, theta_m(i), delta_m(i));    % mass above 0 (posterior-like)
    ...
    % update violation probabilities for the remaining candidates
    if (nstar - set_num) > 0
...
% ============================================================
% 3) Final large Gibbs sampling on full test grid
% ============================================================
cov_X  = feval(covfunc{:}, loghyper, t1);
[~, ~, K3, KK2] = covGP([log(ell);log(sigma_f)], t1, t2);
...
[Ks1, Ks2, Ks3, KKs2] = covGP([log(ell);log(sigma_f)], tstar, t2); %#ok<ASGLU>
...
% 4) Summary metrics
% ============================================================
Zval = Z(:, 5001:10000);            % posterior draws after burn-in
Ef   = median(Zval, 2);             % posterior median
Varf = var(Zval, 0, 2);             % posterior variance
