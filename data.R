...
# 选择一个真函数（可按需切换）
#f_true <- function(x) x + sin(x-10)                                   # 线性+正弦
#f_true <- function(x) 4 / (1 + exp(-(x - 10)/2 + 4))                 # 0–20 上 sigmoid
f_true <- function(x) ifelse(x < 5, x, ifelse(x > 15, x - 10, 5))  # 软阈值
...
make_datasets_csv(R = 25, n = 50, nstar = 100, sigma = 0.5,
                  f_true = f_true, out_dir = "datasets_stepwise_1_csv")
