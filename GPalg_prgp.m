...
train_data = readtable("C:/Users/11613/Downloads/car_train.csv");
xtrain = train_data.xtrain;
ytrain = train_data.ytrain;
% 对 xtrain 进行升序排序，并同步排序 ytrain
[xtrain_sorted, sort_indices] = sort(xtrain);  % 对 xtrain 进行升序排序，并获取排序的索引
ytrain_sorted = ytrain(sort_indices);  % 使用排序索引对 ytrain 进行重新排序




% 每隔10个样本筛选一个
selected_indices = 1:3:length(xtrain_sorted);  % 每隔10个样本选择一个
t1 = xtrain_sorted(selected_indices);  % 构建新的训练集 xtrain
x = ytrain_sorted(selected_indices);  % 构建新的训练集 ytrain
x = -x;
%t1 = train_data.xtrain;
n = length(t1);
%x = train_data.ytrain;
%tstar=linspace(-10,10,nstar)';
data_test = readtable("C:/Users/11613/Downloads/car_test.csv");
tstar =data_test.xtest;
nstar=length(tstar);
